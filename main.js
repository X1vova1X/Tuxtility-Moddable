/*
    Tuxtility Moddable is a Tuxtility version that is NOT MADE for regular use.
    It is made only for flavor development purposes.
    Do NOT use it in your regular projects.
    Based on Tuxtility Base 2.4.
    Wiki: https://github.com/X1vova1X/Tuxtility-Moddable/wiki
    Add custom blocks at line 298. You can't add custom menus (dropdowns) yet.
*/

function registerBlock(id, blockType, text, monitor, arguments) {
    return {
        opcode: id,
        blockType: blockType,
        text: text,
...
function registerArgument(argumentType, defaultValue = "", menu = False) {
    if (menu) {
        return {
...
function tpm(permission) {
    return confirm(`This site wants to have permission for: ${permission}.\nAllow?`);
}

let blocks = []
// DO NOT USE THE BLOCKS.PUSH BELOW TO ADD BLOCKS, THESE ARE DEFAULT TUXTILITY BLOCKS
blocks.push({
    opcode: 'CREDIT',
...
// ADD BLOCKS HERE:
blocks.push(
    // ADD YOUR BLOCKS HERE USING registerBlock FUNCTION
);
...
(function (Scratch) {
    ...
    const vm = Scratch.vm;
    class TuxtiltiyJSModdable {
        getInfo() {
            return {
                id: 'TUXJSMOD',
                name: 'Tuxtility Moddable',
                color1: '#8f8f8f',
                color2: '#2b2b2b',
                blocks: blocks,
                menus: {
        ...
        CREDIT() {
            alert("This extension is based on Tuxtility Moddable - a base for other extensions!")
        }
        NOTIFYALLOW() {
    ...
    Scratch.extensions.register(new TuxtiltiyJSModdable());
})(Scratch);
